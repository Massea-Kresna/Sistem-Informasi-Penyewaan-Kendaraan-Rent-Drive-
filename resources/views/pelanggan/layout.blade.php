...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        :root {
            /* Palet Warna Khusus yang Anda Minta */
            --blue-deep:   #0f2d5e;
            --blue-mid:    #1a4080;
            --blue-accent: #2563eb;
            --blue-soft:   #dbeafe;
            ...
            /* Warna Pendukung untuk Alert & Background Komponen */
            --white:       #ffffff;
            --green:       #16a34a;
            --green-soft:  #f0fdf4;
            --amber:       #d97706;
        ...
        .brand-mark {
            width: 26px; height: 26px; background: var(--blue-deep);
        ...
        .nav-link {
            ...
            transition: color 0.15s, background 0.15s; white-space: nowrap; position: relative;
        }
        .nav-link svg { width: 14px; height: 14px; flex-shrink: 0; opacity: 0.7; }
        .nav-link:hover { color: var(--ink); background: var(--bg); }
        .nav-link.active { color: var(--blue-deep); font-weight: 500; }
        ...
        .navbar-right { display: flex; align-items: center; gap: 1.25rem; margin-left: auto; flex-shrink: 0; }
        .user-chip { display: flex; align-items: center; gap: 8px; }
        .user-avatar {
            width: 28px; height: 28px; background: var(--blue-soft);
            border: 1px solid rgba(37,99,235,0.2); display: flex; align-items: center; justify-content: center;
            font-size: 10px; font-weight: 600; color: var(--blue-deep); letter-spacing: 0.04em; flex-shrink: 0;
        }
        ...
        .user-name { font-size: 12.5px; font-weight: 600; color: var(--ink); display: block; }
        
        /* Logout Button */
        .btn-logout {
            display: flex; align-items: center; gap: 7px; padding: 7px 16px;
            background: transparent; border: 1px solid var(--line);
            font-family: 'Sora', sans-serif; font-size: 11.5px; font-weight: 500;
        ...
        .nav-toggle {
            ...
            padding: 6px 9px; cursor: pointer; margin-left: auto; color: var(--ink-muted);
        }
        ...
        @media (max-width: 820px) {
            ...
            .nav-links.open {
                ...
                position: absolute; top: 60px; left: 0; right: 0; background: var(--white);
                border-bottom: 1px solid var(--line); padding: 0.75rem 1rem; gap: 2px; z-index: 99;
            }
            .nav-links.open + .navbar-right {
                display: flex; position: absolute; top: auto; flex-direction: column;
                align-items: stretch; left: 0; right: 0;
            }
            .nav-link.active::after { display: none; }
            .nav-link.active { background: var(--blue-soft); }
            .mobile-user-row { display: flex !important; }
        }

        .mobile-user-row {
            display: none; align-items: center; justify-content: space-between;
            padding: 0.75rem 1rem; border-top: 1px solid var(--line); background: var(--white);
        }

        /* ─── PAGE CONTENT ─── */
        .page-body { flex: 1; max-width: 1400px; width: 100%; margin: 0 auto; padding: 0; }

        /* ─── FLASH ALERTS ─── */
        .flash-zone { max-width: 1400px; margin: 0 auto; padding: 1rem 2rem 0; }
        .flash {
            display: flex; align-items: flex-start; gap: 10px; padding: 12px 16px;
            font-size: 13px; font-weight: 400; border-left: 3px solid; margin-bottom: 1px;
        }
        ...
        .flash-close {
            ...
            color: inherit; opacity: 0.5; padding: 0; line-height: 1; font-size: 16px;
        }
    ...
    <nav class="navbar" role="navigation" aria-label="Navigasi utama">
        <div class="navbar-inner">
            ...
            <div class="nav-links" id="navLinks">
                ...
                <a href="{{ route('pelanggan.mobil') }}" class="nav-link {{ request()->routeIs('pelanggan.mobil*') ? 'active' : '' }}">
                ...
                <a href="{{ route('pelanggan.riwayat') }}" class="nav-link {{ request()->routeIs('pelanggan.riwayat*') ? 'active' : '' }}">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                    Riwayat Sewa
                </a>
                <a href="{{ route('pelanggan.profil') }}" class="nav-link {{ request()->routeIs('pelanggan.profil*') ? 'active' : '' }}">
            ...
            <div class="navbar-right">
                <div class="user-chip">
                    <div class="user-avatar">
                        {{ strtoupper(substr(session('pelanggan_nama', 'U'), 0, 2)) }}
                    </div>
                    <div class="user-info">
                        <span class="user-label">Pelanggan</span>
                        <span class="user-name">{{ session('pelanggan_nama') }}</span>
                    </div>
                </div>
        ...
        <div class="mobile-user-row" id="mobileUserRow" style="display:none;">
            <div class="user-chip">
                <div class="user-avatar">{{ strtoupper(substr(session('pelanggan_nama', 'U'), 0, 2)) }}</div>
                <div class="user-info">
                    <span class="user-label">Pelanggan</span>
                    <span class="user-name">{{ session('pelanggan_nama') }}</span>
                </div>
